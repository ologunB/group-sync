...
function assertStatus(actual, expected, context = '') {
    assert(actual === expected, `Expected HTTP ${expected}, got ${actual}${context ? ` (${context})` : ''}`);
}
...
const del = (path, token, body) => request('DELETE', path, body, token);
const results = [];
...
async function runAuthSuite() {
    ...
    await test('change-password with wrong old password returns 400', async () => {
        const { status } = await post('/auth/change-password', { old_password: 'WrongOld123', new_password: CHANGED_PASSWORD }, accessToken);
        assertStatus(status, 400);
    });
    await test('change-password with correct credentials returns 200', async () => {
        const { status } = await post('/auth/change-password', { old_password: NEW_PASSWORD, new_password: CHANGED_PASSWORD }, accessToken);
        assertStatus(status, 200);
    });
    await test('change-password with weak new_password (no uppercase) returns 422', async () => {
        const { status } = await post('/auth/change-password', { old_password: CHANGED_PASSWORD, new_password: 'alllower1' }, accessToken);
        assert(status >= 400 && status < 500, `Expected 4xx, got ${status}`);
    });
    await test('change-password with new_password too short returns 422', async () => {
        const { status } = await post('/auth/change-password', { old_password: CHANGED_PASSWORD, new_password: 'Ab1' }, accessToken);
        assert(status >= 400 && status < 500, `Expected 4xx, got ${status}`);
    });
...
async function runFeaturesSuite() {
    ...
    await test('re-login all actors to get fresh tokens reflecting DB change', async () => {
        ...
        creatorToken = cd.data.tokens.accessToken;
        memberToken = md.data.tokens.accessToken;
        outsiderToken = od.data.tokens.accessToken;
    ...
    await test('PATCH /users/me updates displayName and bio (200)', async () => {
        const { status, data } = await patch('/users/me', { display_name: 'Creator Updated', bio: 'Integration test bio' }, creatorToken);
        assertStatus(status, 200);
        const user = data.data;
    ...
    await test('PATCH /users/me with city, state, country updates location fields (200)', async () => {
        const { status, data } = await patch('/users/me', { city: 'Lagos', state: 'Lagos State', country: 'NG' }, creatorToken);
        assertStatus(status, 200);
        const user = data.data;
    ...
    await test('PATCH /users/me with valid HTTPS profile_photo_url (200)', async () => {
        const { status, data } = await patch('/users/me', { profile_photo_url: 'https://example.com/photo.jpg' }, creatorToken);
        assertStatus(status, 200);
        const user = data.data;
    ...
    await test('PATCH /users/me with HTTP (non-HTTPS) profile_photo_url returns 422', async () => {
        const { status } = await patch('/users/me', { profile_photo_url: 'http://example.com/photo.jpg' }, creatorToken);
        assert(status >= 400 && status < 500, `Expected 4xx, got ${status}`);
    });
    ...
    await test('PATCH /users/me with preferred_language sets language (200)', async () => {
        const { status, data } = await patch('/users/me', { preferred_language: 'yo' }, creatorToken);
        assertStatus(status, 200);
        const user = data.data;
    ...
    await test('POST /users/me/interests sets interests array (200)', async () => {
        const { status, data } = await post('/users/me/interests', { interests: ['cooking', 'hiking', 'technology'] }, creatorToken);
        assertStatus(status, 200);
    ...
    await test('POST /users/me/interests deduplicates tags (200)', async () => {
        const { status, data } = await post('/users/me/interests', { interests: ['Cooking', 'cooking', 'COOKING', 'hiking'] }, creatorToken);
        assertStatus(status, 200);
        const interests = data.data.interests;
        assert(interests.filter((i) => i === 'cooking').length === 1, `duplicates not deduped: ${JSON.stringify(interests)}`);
    });
    ...
    await test('POST /users/me/interests with tag exceeding 50 chars returns 422', async () => {
        const { status } = await post('/users/me/interests', { interests: ['x'.repeat(51)] }, creatorToken);
        assert(status >= 400 && status < 500, `Expected 4xx, got ${status}`);
    });
    ...
    await test('POST /groups creates open group (201)', async () => {
        const { status, data } = await post('/groups', { name: `OpenGroup ${ts}`, category: 'Technology', description: 'An open group for integration testing', membership_type: 'open' }, creatorToken);
        assertStatus(status, 201);
    ...
    await test('POST /groups with invalid founding_date returns 422', async () => {
        const { status } = await post('/groups', { name: 'Date Group', category: 'Tech', founding_date: 'not-a-date' }, creatorToken);
        assert(status >= 400 && status < 500, `Expected 4xx, got ${status}`);
    });
    await test('POST /groups with HTTP cover_image_url (not HTTPS) returns 422', async () => {
        const { status } = await post('/groups', { name: 'Cover Group', category: 'Tech', cover_image_url: 'http://example.com/img.jpg' }, creatorToken);
        assert(status >= 400 && status < 500, `Expected 4xx, got ${status}`);
    });
    await test('POST /groups with invalid membership_type returns 422', async () => {
        const { status } = await post('/groups', { name: 'Bad Type', category: 'Tech', membership_type: 'vip_only' }, creatorToken);
        assert(status >= 400 && status < 500, `Expected 4xx, got ${status}`);
    });
    await test('POST /groups with invalid lat returns 422', async () => {
        const { status } = await post('/groups', { name: 'Bad Lat', category: 'Tech', lat: 91, lng: 3.0 }, creatorToken);
        assert(status >= 400 && status < 500, `Expected 4xx, got ${status}`);
    });
    await test('POST /groups creates invite_only group and disables discoverability (201)', async () => {
        const { status, data } = await post('/groups', { name: `InviteGroup ${ts}`, category: 'Sports', membership_type: 'invite_only' }, creatorToken);
        assertStatus(status, 201);
        const group = data.data;
    ...
    await test('GET /groups/:slug includes callerMembershipStatus when authed (200)', async () => {
        ...
        const cms = result.callerMembershipStatus;
    ...
    await test('PATCH /groups/:id updates description only (200, creator = super_admin)', async () => {
        const { status, data } = await patch(`/groups/${openGroupId}`, { description: 'Updated via integration test' }, creatorToken);
        assertStatus(status, 200);
        const group = data.data;
    ...
    await test('PATCH /groups/:id updates name and regenerates slug (200)', async () => {
        ...
        const { status, data } = await patch(`/groups/${openGroupId}`, { name: newName }, creatorToken);
        assertStatus(status, 200);
        const group = data.data;
        assert(group.name === newName, `name not updated: ${group.name}`);
        assert(typeof group.slug === 'string' && group.slug.length > 0, 'slug should be regenerated');
        openGroupSlug = group.slug;
    });
    await test('PATCH /groups/:id updates lat and lng (200)', async () => {
        const { status } = await patch(`/groups/${openGroupId}`, { lat: 6.5244, lng: 3.3792 }, creatorToken);
        assertStatus(status, 200);
    });
    await test('PATCH /groups/:id changing membership_type to invite_only sets isDiscoverable=false (200)', async () => {
        const { status, data } = await patch(`/groups/${openGroupId}`, { membership_type: 'invite_only' }, creatorToken);
        assertStatus(status, 200);
        const group = data.data;
    ...
    await test('PATCH /groups/:id changing from invite_only back to open restores isDiscoverable (200)', async () => {
        const { status, data } = await patch(`/groups/${openGroupId}`, { membership_type: 'open' }, creatorToken);
        assertStatus(status, 200);
        const group = data.data;
    ...
    await test('GET /groups/:id/members returns list with creator (200)', async () => {
        ...
        const found = members.some((m) => m.userId === creatorId || m.user_id === creatorId);
    ...
    await test('GET /groups/:id/members?role=super_admin filters by role (200)', async () => {
        ...
        assert(members.every((m) => m.role === 'super_admin'), 'all returned members should be super_admin');
    });
    ...
    await test('GET /groups/:id/members after join shows new member (200)', async () => {
        ...
        assertStatus(status, 200);
        const members = extractList(data.data);
        const found = members.some((m) => m.userId === memberId || m.user_id === memberId);
    ...
    await test('GET /users/me/groups shows joined group (200)', async () => {
        ...
        const found = groups.some((g) => g.groupId === openGroupId || g.group_id === openGroupId || g.group?.id === openGroupId || g.id === openGroupId);
    ...
    await test('POST /groups/:id/apply to non-existent group returns 404', async () => {
        const { status } = await post('/groups/00000000-0000-4000-8000-000000000000/apply', {}, memberToken);
        assertStatus(status, 404);
    });
    ...
    await test('PATCH /groups/applications/:id rejects application (200)', async () => {
        const { status } = await patch(`/groups/applications/${applicationId}`, { action: 'reject', rejection_reason: 'Not a good fit for now.' }, creatorToken);
        assertStatus(status, 200);
    });
    await test('PATCH /groups/applications/:id as non-admin returns 403', async () => {
        const { status } = await patch(`/groups/applications/${applicationId}`, { action: 'approve' }, outsiderToken);
        assertStatus(status, 403);
    });
    await test('PATCH /groups/applications/:id invalid action returns 422', async () => {
        const { status } = await patch(`/groups/applications/${applicationId}`, { action: 'vanish' }, creatorToken);
        assert(status >= 400 && status < 500, `Expected 4xx, got ${status}`);
    });
    ...
    await test('POST apply + PATCH approve → member becomes active (201 + 200)', async () => {
        ...
        assertStatus(s1, 201);
        const freshAppId = d1.data.applicationId;
        assert(freshAppId?.length > 0, 'freshAppId should be set');
        approvedAppId = freshAppId;
        const { status: s2 } = await patch(`/groups/applications/${freshAppId}`, { action: 'approve' }, creatorToken);
        assertStatus(s2, 200);
    ...
    await test('PATCH /groups/applications/:id for non-existent application returns 404', async () => {
        const { status } = await patch('/groups/applications/00000000-0000-4000-8000-000000000000', { action: 'approve' }, creatorToken);
        assertStatus(status, 404);
    });
    await test('PATCH /groups/applications/:id for already-reviewed application returns 409', async () => {
        ...
        const { status } = await patch(`/groups/applications/${approvedAppId}`, { action: 'reject' }, creatorToken);
        assertStatus(status, 409);
    });
    await test('GET /groups/:id/applications?status=approved returns filtered list (200)', async () => {
        const { status, data } = await get(`/groups/${appGroupId}/applications?status=approved`, creatorToken);
        assertStatus(status, 200);
        const apps = extractList(data.data);
        assert(apps.every((a) => a.status === 'approved'), 'all returned applications should have status=approved');
    });
    await test('GET /groups/:id/applications?status=withdrawn returns filtered list (200)', async () => {
        const { status } = await get(`/groups/${appGroupId}/applications?status=withdrawn`, creatorToken);
        assertStatus(status, 200);
    });
    await test('GET /groups/:id/applications?page=1&limit=1 paginates correctly (200)', async () => {
        const { status, data } = await get(`/groups/${appGroupId}/applications?page=1&limit=1`, creatorToken);
        assertStatus(status, 200);
        const apps = extractList(data.data);
    ...
    await test('GET /users/me/applications?status=withdrawn returns withdrawn applications (200)', async () => {
        ...
        assertStatus(status, 200);
        const apps = extractList(data.data);
        assert(apps.every((a) => a.status === 'withdrawn'), 'all returned applications should be withdrawn');
    });
    ...
    await test('DELETE /groups/applications/:id for non-existent application returns 404', async () => {
        const { status } = await del('/groups/applications/00000000-0000-4000-8000-000000000000', memberToken);
        assertStatus(status, 404);
    });
    ...
    await test('PUT /groups/:id/form as non-admin returns 403', async () => {
        const { status } = await put(`/groups/${appGroupId}/form`, { fields: [{ id: 'q1', type: 'text', label: 'Why?', required: true }] }, outsiderToken);
        assertStatus(status, 403);
    });
    await test('PUT /groups/:id/form creates form with 2 fields (200)', async () => {
        const { status, data } = await put(`/groups/${appGroupId}/form`, {
            fields: [
    ...
    await test('PUT /groups/:id/form replaces form (1 field) (200)', async () => {
        const { status, data } = await put(`/groups/${appGroupId}/form`, { fields: [{ id: 'q1', type: 'textarea', label: 'Tell us about yourself', required: true }] }, creatorToken);
        assertStatus(status, 200);
    ...
    await test('PUT /groups/:id/form with invalid field type returns 422', async () => {
        const { status } = await put(`/groups/${appGroupId}/form`, { fields: [{ id: 'q1', type: 'invalid_type', label: 'Bad', required: true }] }, creatorToken);
        assert(status >= 400 && status < 500, `Expected 4xx, got ${status}`);
    });
    ...
    await test('PUT /groups/:id/form — re-set 1 required field for apply tests (200)', async () => {
        const { status } = await put(`/groups/${appGroupId}/form`, { fields: [{ id: 'q1', type: 'textarea', label: 'Tell us about yourself', required: true }] }, creatorToken);
        assertStatus(status, 200);
    });
    await test('PUT /groups/:id/form with field missing label returns 422', async () => {
        const { status } = await put(`/groups/${appGroupId}/form`, { fields: [{ id: 'q1', type: 'text', required: true }] }, creatorToken);
        assert(status >= 400 && status < 500, `Expected 4xx, got ${status}`);
    });
    await test('PUT /groups/:id/form with field missing id returns 422', async () => {
        const { status } = await put(`/groups/${appGroupId}/form`, { fields: [{ type: 'text', label: 'No ID field', required: true }] }, creatorToken);
        assert(status >= 400 && status < 500, `Expected 4xx, got ${status}`);
    });
    await test('PUT /groups/:id/form with field missing required flag returns 422', async () => {
        const { status } = await put(`/groups/${appGroupId}/form`, { fields: [{ id: 'q1', type: 'text', label: 'No required flag' }] }, creatorToken);
        assert(status >= 400 && status < 500, `Expected 4xx, got ${status}`);
    });
    ...
    await test('POST /groups/:id/apply with all required form fields returns 201', async () => {
        const { status } = await post(`/groups/${appGroupId}/apply`, { form_responses: { q1: 'I am very passionate about this topic and would love to contribute.' } }, outsiderToken);
        assertStatus(status, 201);
    });
    ...
    await test('PATCH /groups/:id/members/:userId promotes to moderator (200)', async () => {
        const { status } = await patch(`/groups/${openGroupId}/members/${memberId}`, { role: 'moderator' }, creatorToken);
        assertStatus(status, 200);
    });
    ...
    await test('PATCH /groups/:id/members/:userId as non-admin returns 403', async () => {
        const { status } = await patch(`/groups/${openGroupId}/members/${memberId}`, { role: 'member' }, outsiderToken);
        assertStatus(status, 403);
    });
    await test('PATCH /groups/:id/members/:userId with no role or status returns 400', async () => {
        const { status } = await patch(`/groups/${openGroupId}/members/${memberId}`, {}, creatorToken);
        assertStatus(status, 400);
    });
    await test('PATCH /groups/:id/members/:userId with invalid role value returns 422', async () => {
        const { status } = await patch(`/groups/${openGroupId}/members/${memberId}`, { role: 'god' }, creatorToken);
        assert(status >= 400 && status < 500, `Expected 4xx, got ${status}`);
    });
    await test('PATCH /groups/:id/members/:userId with invalid status value returns 422', async () => {
        const { status } = await patch(`/groups/${openGroupId}/members/${memberId}`, { status: 'deleted' }, creatorToken);
        assert(status >= 400 && status < 500, `Expected 4xx, got ${status}`);
    });
    await test('PATCH /groups/:id/members/:userId targeting super_admin returns 403', async () => {
        ...
        const { status } = await patch(`/groups/${openGroupId}/members/${creatorId}`, { role: 'admin' }, creatorToken);
        assertStatus(status, 403);
    });
    await test('PATCH /groups/:id/members/:userId for non-existent member returns 404', async () => {
        const { status } = await patch(`/groups/${openGroupId}/members/00000000-0000-4000-8000-000000000000`, { role: 'member' }, creatorToken);
        assertStatus(status, 404);
    });
    ...
    await test('POST /groups/:id/invite generates link (201)', async () => {
        const { status, data } = await post(`/groups/${openGroupId}/invite`, { max_uses: 10, expires_in_hours: 48 }, creatorToken);
        assertStatus(status, 201);
    ...
    await test('POST /groups/:id/invite with expires_in_hours > 8760 returns 422', async () => {
        const { status } = await post(`/groups/${openGroupId}/invite`, { expires_in_hours: 99999 }, creatorToken);
        assert(status >= 400 && status < 500, `Expected 4xx, got ${status}`);
    });
    await test('POST /groups/:id/invite with max_uses=0 returns 422', async () => {
        const { status } = await post(`/groups/${openGroupId}/invite`, { max_uses: 0 }, creatorToken);
        assert(status >= 400 && status < 500, `Expected 4xx, got ${status}`);
    });
    ...
    const TINY_PNG = Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAAC0lEQVQI12NgAAIABQ' +
    ...
    async function uploadFile(path, fieldName, buffer, mimeType, token) {
    ...
    await test('POST /users/me/photo with valid PNG uploads and returns URL (200)', async () => {
        ...
        assertStatus(status, 200);
        const d = data.data;
        assert(typeof d?.url === 'string' && d.url.startsWith('https://'), 'Expected url to be an HTTPS string');
    });
    // ── Group cover ──────────────────────────────────────────────────────────
    ...
    await test('POST /groups/:id/events as member (non-admin) returns 403', async () => {
        const { status } = await post(`/groups/${openGroupId}/events`, { title: 'Test Event', starts_at: new Date(Date.now() + 3_600_000).toISOString() }, memberToken);
        assertStatus(status, 403);
    });
    await test('POST /groups/:id/events with past starts_at returns 422', async () => {
        const { status } = await post(`/groups/${openGroupId}/events`, { title: 'Past Event', starts_at: '2020-01-01T00:00:00Z' }, creatorToken);
        assertStatus(status, 422);
    });
    await test('POST /groups/:id/events with valid data creates event (201)', async () => {
        const future = new Date(Date.now() + 3_600_000).toISOString();
        const { status, data } = await post(`/groups/${openGroupId}/events`, { title: 'Awesome Meetup', description: 'Fun event', starts_at: future, rsvp_limit: 50 }, creatorToken);
        assertStatus(status, 201);
        const d = data.data;
    ...
    await test('PATCH /notifications/preferences updates preferences (200)', async () => {
        const { status, data } = await patch('/notifications/preferences', { preferences: [{ pref_type: 'event_created', push_enabled: false, in_app_enabled: true }] }, creatorToken);
        assertStatus(status, 200);
    ...
    await test('POST /reports with invalid reason returns 422', async () => {
        const { status } = await post('/reports', { target_type: 'user', target_id: memberId, reason: 'invalid_reason' }, creatorToken);
        assertStatus(status, 422);
    });
    await test('POST /reports with invalid target_type returns 422', async () => {
        const { status } = await post('/reports', { target_type: 'banana', target_id: memberId, reason: 'spam' }, creatorToken);
        assertStatus(status, 422);
    });
    await test('POST /reports with valid data submits report (201)', async () => {
        const { status, data } = await post('/reports', { target_type: 'user', target_id: memberId, reason: 'spam', description: 'Test report' }, creatorToken);
        assertStatus(status, 201);
        const d = data.data;
    ...
    const superAdminToken = encryption_1.EncryptionUtil.generateJWT({
        userId: creatorId,
        role: 'super_admin',
    ...
    await test('PATCH /admin/users/:id/verification approves ID (200)', async () => {
        const { status } = await patch(`/admin/users/${memberId}/verification`, { decision: 'approved' }, adminToken);
        assertStatus(status, 200);
    });
    ...
    await test('GET /groups/:id/messages as member returns list (200)', async () => {
        ...
        assert(data.data.length >= 1, 'expected at least one message');
    });
    ...
    await test('GET /groups/:id/messages/pinned returns pinned list (200)', async () => {
        ...
        assertStatus(status, 200);
        assert(Array.isArray(data.data), 'expected array');
        assert(data.data.some((m) => m.id === pinnedMessageId), 'pinned message not in list');
    });
    ...
    await test('PATCH /groups/:id/chat as admin locks chat (200)', async () => {
        ...
        assertStatus(status, 200);
        assert(data.data.is_chat_locked === true, 'expected chat to be locked');
    });
    ...
    await test('PATCH /groups/:id/chat unlocks chat (200)', async () => {
        ...
        assertStatus(status, 200);
        assert(data.data.is_chat_locked === false, 'expected chat to be unlocked');
    });
    ...
    await test('GET /dm/:userId returns thread (200)', async () => {
        ...
        assertStatus(status, 200);
        assert(Array.isArray(data.data), 'expected array');
        assert(data.data.some((m) => m.id === dmId), 'sent DM not in thread');
    });
    await test('PATCH /dm/:userId/read marks thread as read (200)', async () => {
        ...
        assertStatus(status, 200);
        assert(typeof data.data.count === 'number', 'expected count field');
    });
    ...
    function connectSocket(token) {
        return new Promise((resolve, reject) => {
            const s = (0, socket_io_client_1.io)(SOCKET_URL, {
                auth: { token },
    ...
    await test('connect without token returns connect_error', async () => {
        await new Promise((resolve, reject) => {
            const s = (0, socket_io_client_1.io)(SOCKET_URL, { transports: ['websocket'], reconnection: false, timeout: 4000 });
    ...
    await test('join_group for unknown group emits error event', async () => {
        ...
        const errPromise = waitForEvent(creatorSocket, socket_events_1.SocketEvents.ERROR);
        creatorSocket.emit(socket_events_1.SocketEvents.JOIN_GROUP, { group_id: fakeId });
    ...
    await test('send_message via socket broadcasts new_message to room members', async () => {
        ...
        const newMsgPromise = waitForEvent(memberSocket, socket_events_1.SocketEvents.NEW_MESSAGE);
        creatorSocket.emit(socket_events_1.SocketEvents.SEND_MESSAGE, {
    ...
    await test('send_message to locked group as non-admin emits error', async () => {
        ...
        const errPromise = waitForEvent(memberSocket, socket_events_1.SocketEvents.ERROR);
        memberSocket.emit(socket_events_1.SocketEvents.SEND_MESSAGE, {
            group_id: openGroupId,
    ...
    await test('user_typing emits typing event to group room', async () => {
        const typingPromise = waitForEvent(memberSocket, socket_events_1.SocketEvents.TYPING);
        creatorSocket.emit(socket_events_1.SocketEvents.USER_TYPING, { group_id: openGroupId });
    ...
    await test('dm_send via socket delivers dm_received to receiver', async () => {
        ...
        creatorSocket.emit(socket_events_1.SocketEvents.DM_SEND, {
    ...
    await test('dm_send to user with no shared group emits error', async () => {
        ...
        const { userId: loneSocketId } = await registerAndLogin(`lone_socket${ts}_${Math.random().toString(36).slice(2, 8)}@test.io`, 'Lone1234!', 'LoneSocket');
        const errPromise = waitForEvent(creatorSocket, socket_events_1.SocketEvents.ERROR);
        creatorSocket.emit(socket_events_1.SocketEvents.DM_SEND, {
    ...
    await test('refresh tokens before deletion tests', async () => {
        ...
        const freshOutsider = od.data?.tokens?.accessToken;
